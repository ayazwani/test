...
def processvid(filename1):
    ...
    vid = cv2.VideoCapture('./static/'+filename1)
    ...
    out = cv2.VideoWriter('./static/processed2.mp4' , codec, fps, (width, height))
...
#r naught
        if(newr>=2):
            cv2.rectangle(img, (int(5), int(height-140)) , ( int(300), int(height-5)),(255,255,255), 2)
...
#average r0
        #tp=masked+unmasked
        if((len(set(countunmask)))+(len(set(countmask)))>0):
            overallpop=(len(set(countunmask)))+(len(set(countmask)))
            npp=(len(set(countmask)))/overallpop
            ef=.50
            AVGRr=5.7*(1-(ef*npp))**2
            cv2.putText(img, "R0 Avg: {:.1f} ".format(AVGRr), (int(10), int(height-120)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2)
